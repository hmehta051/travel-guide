export const formatTimestamp = (timestamp: string | number): string => {
  const currentTime: number = Math.floor(Date.now() / 1000); // Get the current timestamp in seconds
  const timeDifference: number = currentTime - +timestamp; // Calculate the time difference in seconds
